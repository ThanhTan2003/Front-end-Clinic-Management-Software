...
import { searchServiceCategories } from '../../../../services/AppointmentService'
import { getToken } from "../../../../services/localStorageService";

import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faCaretDown, faMagnifyingGlass, faFileMedical } from "@fortawesome/free-solid-svg-icons";
import ModalDetail from '../Compoments/Booking/ModalDetail';  // Import component ModalDetail
...
export default function DanhSachDichVu() {
    ...
    const fetchServiceCategories = async (accessToken) => {
        ...
        try {
            const response = await fetch(
                `${searchServiceCategories}?keyword=${keyword}&page=${currentPage}&size=${pageSize}`,
                {
                    method: "GET",
                    headers: {
            ...
            if (currentPage === 1) {
                setServiceCategories(data.data || []); // Nếu là trang đầu tiên, thay thế danh sách
            } else {
                setServiceCategories(prevCategories => [...prevCategories, ...(data.data || [])]); // Nếu là các trang tiếp theo, thêm mới vào
            }
    ...
    const openModal = (category) => {
        setModalContent({ title: category.categoryName.toUpperCase(), content: category.description });
                            ...
                            {serviceCategories.map((category) => (
                                ...
                                >
                                    <div className="flex items-center gap-6 mb-6">
                                        <img
                                            src={category.image || "https://cdn-icons-png.flaticon.com/512/1976/1976945.png"}
                                            alt="service-icon"
